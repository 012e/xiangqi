...
export const getHealthAuthQueryOptions = <
  TData = Awaited<ReturnType<typeof healthAuth>>,
  TError = unknown,
>(options?: {
  query?: Partial<
    UseQueryOptions<Awaited<ReturnType<typeof healthAuth>>, TError, TData>
  >;
  request?: SecondParameter<typeof customInstance>;
}) => {
  const { query: queryOptions, request: requestOptions } = options ?? {};
  ...
  const queryFn: QueryFunction<Awaited<ReturnType<typeof healthAuth>>> = ({
    signal,
  }) => healthAuth(requestOptions, signal);
...
export type HealthAuthQueryResult = NonNullable<
  Awaited<ReturnType<typeof healthAuth>>
>;
