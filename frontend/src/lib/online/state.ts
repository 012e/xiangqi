...
type GameEndReason =
    | 'black_resign' | 'black_timeout' | 'black_checkmate'
    | 'white_resign' | 'white_timeout' | 'white_checkmate'
    | 'stalemate' | 'insufficient_material' | 'fifty_move_rule' | 'mutual_agreement';
...
export class StatePlay implements BaseState<'State.Play', {
    from: string;
    to: string;
    player: PlayerColor;
}> {
...
export class StateError implements BaseState<'State.Error', { message: string }> {
...
export class StateGameEnd implements BaseState<'State.GameEnd', {
